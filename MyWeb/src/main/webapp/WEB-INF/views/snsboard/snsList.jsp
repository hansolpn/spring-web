			...
			<div class="row">
				<aside class="col-sm-2">
					<div class="aside-inner">
						<div class="menu1">
							<p>
								<c:if test="${sessionScope.login != unll}"><img src="${pageContext.request.contextPath}/img/profile.png"> ${sessionScope.login}</c:if> 
								<c:if test="${sessionScope.login == null}"><strong>로그인을 해 주세요!</strong></c:if> 
							</p>
							<ul>
								<li>사이드메뉴1</li>
								<li>사이드메뉴2</li>
		...
		window.onload = () => {
			...
			function regist() {
				...
				*/
				...
				//한 화면에 파일 업로드 버튼이 여러 개일 경우 요소의 인덱스를 지목해서 가져	올 수 있음.
				...
				// FormData 객체를 보낼 때는 따라 headers 설정을 진행하지 않습니다.
				fetch('${pageContext.request.contextPath}/snsboard', {
					method: 'post',
					body: formData
				})
				// 응답데이터는 text로 전달됩니다. (uploadSuccess)
				.then(res => res.text())
				.then(data => {
					...
					// file input 내용을 비워주시고, 글 영역 비워 주시고,
