...
public class ReplyController {
	...
	// 댓글 등록
	@PostMapping()
	public String replyRegist(@RequestBody ReplyRegistRequestDTO dto) {
	...
	public Map<String, Object> getList(@PathVariable int bno, @PathVariable int pageNum) {
		/*
		 1. 화면단에서 getList메서드가 글 번호, 페이지 번호를 url을 통해 전달합니다.
		 ...
		 (댓글 목록 리스트와 전체 댓글 개수를 함께 전달할 예정.) -> 일회성으로 쓸거니까 Map으로 클라이언트에게 전달.
		 */
		
		System.out.println("/list/" + bno + "/" + pageNum);
		List<ReplyListResponseDTO> list = service.getList(bno, pageNum);
	...
	// 댓글 수정 요청
	@PutMapping("/{rno}")
	public String update(@PathVariable int rno, @RequestBody ReplyRequestDTO dto) {
		dto.setReplyNo(rno);
	...
	// 댓글 삭제 요청
	@DeleteMapping("/{rno}")
	public String delete(@PathVariable int rno, @RequestBody ReplyRequestDTO dto) {
		dto.setReplyNo(rno);
